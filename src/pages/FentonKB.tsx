...
function Md({ text }: { text: string }) {
  ...
  const flush = () => { if (!ul.length) return; out.push(<ul key={out.length} className="my-1 space-y-1">{ul.map((it, i) =>
    <li key={i} className="flex gap-2 text-[12px] text-[var(--t2)] leading-relaxed"><span className="text-[var(--violet)] shrink-0 mt-0.5">•</span><span>{inline(it)}</span></li>)}</ul>); ul = []; };
  ...
  return (
    <div className="space-y-[17px]">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="w-9 h-9 rounded-xl bg-[var(--warn-soft)] ring-1 ring-inset ring-[var(--warn-soft)] flex items-center justify-center shrink-0">
          <Lightbulb className="w-4.5 h-4.5 text-[var(--warn)]" />
        </div>
        ...
        {cards.length > 0 && (
          <button aria-label="Re-run AI extraction on all emails" onClick={() => refresh(true)} disabled={refreshing} title="Re-run AI extraction on all emails"
            className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-[var(--r-sm)] text-[11.5px] font-medium ring-1 ring-inset ring-[var(--line-2)] text-[var(--t2)] hover:bg-[var(--s3)] disabled:opacity-50 transition-colors">
      ...
      {cards.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-[17px] items-start">
          ...
          <div className="lg:col-span-3 space-y-2.5">
            <div className="flex items-center gap-1.5 h-8 px-2.5 rounded-[var(--r-sm)] bg-[var(--s3)] ring-1 ring-inset ring-[var(--line)] focus-within:ring-[var(--violet)]">
              ...
              return (
                <div key={c.entryId} className="rounded-xl ring-1 ring-inset ring-[var(--line)] bg-[var(--s1)] p-3.5">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 mt-2">
                    {(c.tags || []).map(t => (
                      <span key={t} className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-[var(--violet-soft)] text-[var(--violet)]">{t}</span>
                    ))}
                    <button onClick={() => toggle(c.entryId)} className="ml-auto inline-flex items-center gap-0.5 text-[10.5px] text-[var(--t3)] hover:text-[var(--violet)] transition-colors">
